...
class TestAPNs(unittest.TestCase):
    ...
    def testPayloadAlert(self):
        ...
        pa = PayloadAlert(
            'foo', action_loc_key='bar', loc_key='wibble', loc_args=['king', 'kong'], launch_image='wobble'
        )
        d = pa.dict()
        self.assertEqual(d['body'], 'foo')
    ...
    def testFrame(self):
        ...
        f1 = bytearray(b'\x02\x00\x00\x00t\x01\x00 \xb5\xbb\x9d\x80\x14\xa0\xf9\xb1\xd6\x1e!\xe7\x96\xd7\x8d\xcc' +
                       b'\xdf\x13R\xf2<\xd3(\x12\xf4\x85\x0b\x87\x8a\xe4\x94L\x02\x00' +
                       b'<' + payload.json() +
                       b'\x03\x00\x04\x00\x00\x00\x01\x04\x00\x04\x00\x00\x0e\x10\x05\x00\x01\n')
        self.assertEqual(f1, frame.get_frame())
    ...
    def testPayloadTooLargeError(self):
        ...
        self.assertRaises(PayloadTooLargeError, Payload, '.' * (max_raw_payload_bytes + 1))

        # Test unicode 2-byte characters payload
        Payload(u'\u0100' * int(max_raw_payload_bytes / 2))
        self.assertRaises(PayloadTooLargeError, Payload, u'\u0100' * (int(max_raw_payload_bytes / 2) + 1))
